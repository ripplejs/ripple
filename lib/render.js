...
module.exports = function(bindings, view) {
  ...
  walk(el, function(node, next){
    if(node.nodeType === 3) {
      view.activeBindings.push(new TextBinding(view, node));
    }
    else if(node.nodeType === 1) {
      var View = bindings.component(node);
      if(View) {
        return view.activeBindings.push(new ChildBinding(view, node, View));
      }
      each(attrs(node), function(attr){
        var binding = bindings.directive(attr);
        if(binding) {
          view.activeBindings.push(new Directive(view, node, attr, binding));
        }
        else {
          view.activeBindings.push(new AttrBinding(view, node, attr));
        }
      });
    }
    next();
